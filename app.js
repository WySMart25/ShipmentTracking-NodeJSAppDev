...
app.post('/checkftpConnection', async (req, res) => {
    const { host, user, password, secure, port } = req.body;

    if (!host || !user || !password) {
        return res.status(400).json({ error: "Missing required parameters: 'host', 'user', 'password'." });
    }
